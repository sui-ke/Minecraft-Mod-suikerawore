...
public class SagMillRecipe {
    ...
    public static void register(Item input, List<String> outputNameList, List<Integer> outputAmount, List<Float> outputChance) {
        ...
        for (int i = 0; i < outputNameList.size(); i++) {
            ...
            /*获取输出物的ItemStack*/ItemStack outputStack = createStack(outputName, amount);

            if (!outputStack.isEmpty()) {//检查outputStack是否为空

                /*添加到输出物列表*/outputList.add(new RecipeOutput(outputStack, chance));
            }
        }

        if (!outputList.isEmpty()) {//检查outputs列表是否为空
        //注册配方
            SagMillRecipeManager.getInstance().addRecipe(
                new Recipe(
                    new RecipeInput(new ItemStack(input)),//输入物品
                    2000,//需要的能量
                    RecipeBonusType.MULTIPLY_OUTPUT,//奖励类型
                    RecipeLevel.SIMPLE,//配方等级
    ...
    //获取对应的 ItemStack 方法
    public static ItemStack createStack(String outputOD, int amount) {
        //获取对应的 ItemStack
