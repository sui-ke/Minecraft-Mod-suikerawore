...
// 溶液
    ...
    public String getFluid() {
        return this.fluid.isEmpty() ? StringUtils.uncapitalize(this.getElementType()) : this.fluid;
    }
...
// 设为有效
    ...
    public static void upRefinedValid() {
        if (ConfigValue.needIngot) {
            synchronized (ModItemRawOre.TYPE_TO_RAW_ORE) {
                for (Map.Entry<String, ModItemRawOre> mapEntry : ModItemRawOre.TYPE_TO_RAW_ORE.entrySet()) {
                    mapEntry.getValue().upMainRefinedValid();
                }
            }
        }
    }

    private void upMainRefinedValid() {
        if (this.isValid() && this.getRefinedFromName().isEmpty()) {
            IItemEntry.setValid(true, this.mainRefined);
        }
    }
