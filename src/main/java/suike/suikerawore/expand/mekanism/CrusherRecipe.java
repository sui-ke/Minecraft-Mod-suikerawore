...
//配方注册
    public static void register(Item input, String outputOD) {
        ...
        ItemStack outputStack = ItemBase.oreStack(outputOD);
        /*设置数量*/outputStack.setCount(ConfigValue.CrusherOutputAmount);

        if (ItemBase.isValidItemStack(inputStack, outputStack)) {
            /*配方注册*/RecipeHandler.addCrusherRecipe(inputStack, outputStack);
        }
    }
}