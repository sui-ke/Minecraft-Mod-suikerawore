...
public class InductionSmelterRecipe {
    public static void register() {
        ...
        /*铍*/register(ItemBase.RAW_BERYLLIUM, "ingotBeryllium");
        /*镭*/register(ItemBase.RAW_IRRADIUM, "ingotIrradium");
        /*钯*/register(ItemBase.RAW_PALLADIUM, "ingotPalladium");
        /*钚*/register(ItemBase.RAW_PLUTONIUM, "ingotPlutonium");
        /*铌*/register(ItemBase.RAW_NIOBIUM, "ingotNiobium");
        /*秘银*/register(ItemBase.RAW_MITHRIL, "ingotMithril");
        /*金红石*/register(ItemBase.RAW_RUTILE, "ingotRutile");
...
//转译变量
    ...
    public static void register(Item inputItem, String outputOreDict) {
        ItemStack itemStack = ItemBase.oreStack(outputOreDict);

        if (ItemBase.isValidItemStack(itemStack)) {
            outputStack = itemStack;
            register(inputItem, null, 0, new ArrayList<>(defaultSecondaryOutputList));
        }
    }

//配方注册
    public static ItemStack outputStack = null;//主输出物
    public static void register(Item inputItem, Item outputItem, int meta, List<String> secondaryOutputList) {
        ...
        for (int i = 0 ; i < outputAmounts.length; i++) {
            if (outputItem == null) {
                if (outputStack == null)
                    return;
                /*主输出物*/outputStack.setCount(outputAmounts[i]);
            } else {
                /*主输出物*/outputStack = new ItemStack(outputItem, outputAmounts[i], meta);
            }
