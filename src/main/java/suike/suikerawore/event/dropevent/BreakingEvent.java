...
public class BreakingEvent {
    ...
    private static ItemStack lastDrop = ItemStack.EMPTY;
    private static IBlockState lastBlock = BlockBase.RAW_BLOCK_GOLD.getDefaultState();
    public static BlockEvent.HarvestDropsEvent dropsEvent;
    ...
    public static void onHarvestDrops(BlockEvent.HarvestDropsEvent event) {
        ...
        if (harvester != null) {
            ...
            // 检查是否是 Hammer Time 模组的锤子
            if (!(HandHold.isHoldingHammertime(event.getPos(), event.getWorld()))) return;
        }
        else {return;} // 非已知破坏方式
    ...
    private static void dropItme(BlockEvent.HarvestDropsEvent event, ItemStack dropStack) {
        // 设置掉落物和最后处理的方块
    ...
    private static void dropItme(BlockEvent.HarvestDropsEvent event) {
        // 获取掉落物
        ItemStack dropStack = BreakingEvent.lastDrop;

        // 获取掉落事件
        BreakingEvent.dropsEvent = event;
