...
public class Drop {
    ...
    public static void handleEntityDrop(ItemStack itemStack) {
        ...
        if (fortuneLevel > 0) {
            if (BreakingEvent.dense) {
                ...
                for (int i = 0; i < denseMultiple; i++) {
                    total += rawOreDropAmount + BreakingEvent.Event.getWorld().rand.nextInt(fortuneLevel + 1);
                }
            } else {
                // 普通矿石时运
                dropAmount += BreakingEvent.Event.getWorld().rand.nextInt(fortuneLevel + 1);
            }
        }
        ...
        // 清除并添加新掉落物
        BreakingEvent.Event.getDrops().clear();
        BreakingEvent.Event.getDrops().add(dropStack);
