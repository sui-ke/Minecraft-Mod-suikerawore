...
//配方定义
    public static void register() {
        ...
        if (Examine.ic2c_extrasID) {
            /*钍*/register("Thorium", Item.getByNameOrId("ic2c_extras:thorium232ingot"), null);
        } else {
        ...
        /*铍*/register(ItemBase.RAW_BERYLLIUM, BlockBase.RAW_BLOCK_BERYLLIUM, "Beryllium");
        /*镭*/register(ItemBase.RAW_IRRADIUM, BlockBase.RAW_BLOCK_IRRADIUM, "Irradium");
        /*钯*/register(ItemBase.RAW_PALLADIUM, BlockBase.RAW_BLOCK_PALLADIUM, "Palladium");
        /*钚*/register(ItemBase.RAW_PLUTONIUM, BlockBase.RAW_BLOCK_PLUTONIUM, "Plutonium");
        /*铌*/register(ItemBase.RAW_NIOBIUM, BlockBase.RAW_BLOCK_NIOBIUM, "Niobium");
        /*秘银*/register(ItemBase.RAW_MITHRIL, BlockBase.RAW_BLOCK_MITHRIL, "Mithril");
        /*金红石*/register(ItemBase.RAW_RUTILE, BlockBase.RAW_BLOCK_RUTILE, "Rutile");
...
// item 输出
    public static void register(String inputOreDict, Item outputItem, Block outputBlock) {
        if (outputItem instanceof ItemBase) {
            if (!ItemBase.ITEMS.contains(outputItem)) return;
        }
        ...
        for (String prefix : prefixes) {
            if (prefix.equals("rawBlock") && (!ConfigValue.rawBlockSmelt || outputBlock == null)) {
            //检查是否打开粗块熔炼
                continue;
            }

            //获取所有与矿词匹配的物品
            NonNullList<ItemStack> inputList = OreDictionary.getOres(prefix + inputOreDict);
            ...
            for (ItemStack input : inputList) {
                ...
                /*注册配方*/register(input, outputStack);
            }
        }
    }

// item 输入
    public static void register(Item inputItem, Block blockInput, String outputOreDict) {
        if (inputItem instanceof ItemBase) {
            if (!ItemBase.ITEMS.contains(inputItem)) return;
        }
        ...
        for (String prefix : prefixes) {
            ...
            ItemStack outputStack = ItemBase.oreStack(prefix + outputOreDict, ConfigValue.FurnaceOutputAmount);

            /*注册配方*/register(inputStack, outputStack);
        }
    }
